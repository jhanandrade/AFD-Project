...
#############################################################
cppMakeSys <- function(fileName=NULL, tderiv=NULL, parms=NULL, times=NULL, y0=NULL,
											 intermediateVar=NULL, eventTime=NULL, eventVar=NULL, compile=TRUE, reportVars=NULL) {
	if (!is.null(fileName)) {
		sys <- loadModel(fileName)
	}
	sys <- completeSys(c("tderiv", "parms", "times", "y0", "intermediateVar", "eventTime", "eventVar","reportVars"), 
...
##################################################
makeCppFunc <- function(allEquations, parms, tderiv,ivvars) { # Create the cpp function defining differential equations for the kernel
	
	# init the string with inputs
	strCppFunc <- 'void Func(float t, float* y, float* parms, float* ydot, float* x) {'
...
cppCompileRK4 <- function(sys=NULL, dim=sys$dim, strFunc=sys$strFunc, eventTime=sys$eventTime,
													eventVar=sys$eventVar, parms=sys$parms, tderiv=sys$tderiv, intermediateVars=sys$intermediateVars, reportVars=sys$reportVars) {
	
	if (is.null(sys))
		sys <-list()
	
	sys <- completeSys(c("dim", "strFunc", "eventTime", "eventVar", "parms", "tderiv", "intermediateVars", "reportVars"), sys, envir=environment())
	...
	strReportingVars <- ifelse(reportVars==0, "", makeReportingVariable(sys))
	...
	cppCode <- sub("@AddEventTime", strEventTime, cppCode)
	cppCode <- sub("@AddEventVar", strEventVar, cppCode)
	cppCode <- sub("@AddReportingVars", strReportingVars, cppCode)
...
################################################################
makeReportingVariable <- function(sys=NULL, strFunc=sys$strFunc, eventVar=sys$eventVar, 
																	parmsNames=names(sys$parms), yNames = names(sys$tderiv), reportVars=sys$reportVars) {
	if(reportVars==3){
		out <- "for(it1=0;it1<dim;it1++){
...
################################################
cppRK4 <- function(sys=NULL, times=NULL, parms=NULL, y0=NULL, eventTime=NULL, eventVar=NULL, reportVars = NULL) {
	if(is.null(sys))
		sys <- list()
	sys <- completeSys(c("parms", "times", "y0", "eventTime", "eventVar", "reportVars"), sys, envir=environment())
	...
	solution = RK4(sys$nT, sys$byT, sys$y0, sys$parms)
